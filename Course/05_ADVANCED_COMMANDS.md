...
* `sort < archivo.txt` Usa el archivo como entrada y ejecuta el comando con su contenido (en este caso, ordenar).
	* `sort archivo.txt` Ordena las líneas de un archivo alfabéticamente.
...
* `cat nombre_archivo | grep "texto a buscar" | wc -w` Muestra el contenido de un archivo, busca un texto en ese contenido y realiza el recuento de palabras resultantes de la búsqueda. Ej: `cat LICENCE | grep "Object" | wc -w`
